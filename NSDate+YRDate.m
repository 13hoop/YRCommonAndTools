...
//  SDBoneRecovery
//
//  Created by Naton on 2018/10/8.
//  Copyright © 2018 13hoop. All rights reserved.
//

#import "NSDate+YRDate.h"
...
#define YR_DATE_SIGN @"yr_date_for_alert_show"
...
@implementation NSDate (YRDate)


+ (BOOL)isNewDay {
    NSDate *date = [NSDate date];
    NSString *dateStr = [date stringWithFormat:@"YYYY-MM-dd"];
    NSString *lastDateStr = [[NSUserDefaults standardUserDefaults] valueForKey:YR_DATE_SIGN];
    if([dateStr isEqualToString:lastDateStr]) {
        return NO;
    }else {
        [[NSUserDefaults standardUserDefaults] setObject:dateStr forKey:YR_DATE_SIGN];
        return YES;
    }
}
...
- (NSString *)getDateStringWithTimeStap:(double)str {
    NSTimeInterval time=str/1000;//传入的时间戳str如果是精确到毫秒的记得要/1000
    NSDate *detailDate=[NSDate dateWithTimeIntervalSince1970:time];
    NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init]; //实例化一个NSDateFormatter对象
    //设定时间格式,这里可以设置成自己需要的格式
    [dateFormatter setDateFormat:@"yyyy-MM-dd HH:mm:ss"];
...
+ (NSDate *)getDateWithTimeStr:(NSString *)dateString usingFormate:(NSString *)format {
